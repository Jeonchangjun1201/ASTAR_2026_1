...
namespace BFS
{
    public class TOWKeyQTEManager : MonoBehaviour                                                                // Key minigame manager script // 키 미니게임 매니저
    {
        ...
        private IEnumerator NextInputCoroutine(ITeamTOW team, bool val)                                          // Coroutine, will give penalty to a player if they messed up with the minigame // 코루틴, 패널티 주는 용도
        {
            ...
            {
                _isPenalty = true;
                Debug.Log($"<color=red>You can't do anything for {_penaltyTime} seconds...</color>");            // TEMPORARY; for debugging, change this to UI when available // 임시, UI로 만들 것
        ...
        {
            Debug.Log(IsCorrect ? "<color=green>SUCCESS!</color>" : "<color=red>WRONG!</color>");                // TEMPORARY; for debugging, shows if its correct or wrong, should be changed to UI //임시, UI로 만들 것
            ...
            moveDir *= 0.2f;                                                                                     // Modifying movement distance // 움직이는 거리 조정
            _rope.MoveRope(IsCorrect ? moveDir :                                                                 // If player pressed correct key, then move it to default direction // 플레이어가 맞는 키를 입력했으면, 기본 방향으로 이동
                _scoreManager.scoreBoard[team.Team] > 0 ? (moveDir * -1) : (moveDir * 0));                       // Else, move rope to opposite direction unless corresponding team has score of 0 // 아니면, 반대 방향으로 이동. 해당하는 팀의 점수가 0이면 움직이지 않음
